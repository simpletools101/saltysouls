...
import { ManagedPromise } from "../../lib/services/async";
...
export const IRequestsContext = createServiceContext<RequestsModel>(
...
export class RequestsModel {
    ...
    private async filterAndCacheData(blogData: IBlogPost[]) {
        return new Promise<void>((c) => {
            ...
            this.cachedRequestsData.bannerBlogCards =
                this.cachedRequestsData.blogData
                    .map((data) => {
                        return {
                            title: data.title,
            ...
            this.cachedRequestsData.articleCards =
                this.cachedRequestsData.blogData.map((data) => {
                    return {
                        title: data.title,
                        createdAt: data.createdAt,
            ...
            this.cachedRequestsData.searchItems =
                this.cachedRequestsData.blogData.map((data) => {
                    return {
                        title: data.title,
            ...
            if (
                this.cachedRequestsData.blogData.length > 0 &&
                this.cachedRequestsData.bannerBlogCards.length > 0
            ) {
                c();
            }
        });
    ...
    private getPrivacyPolicyItems() {
        if (this.hasCachedData(this.cachedRequestsData.privacyPolicies)) {
            this.__privacyPoliciesPromise.__completeWithValue(
                this.cachedRequestsData.privacyPolicies
            );
        } else {
    ...
    public async getContentDataForBlog(slug: string) {
        ...
        if (this.hasCachedData(this.cachedRequestsData.blogData)) {
            let blogItems: IBlogContentItem[] =
                this.cachedRequestsData.blogData.filter((v) => {
                    return v.slug == slug;
                });
    ...
    private getBlogArticles() {
        if (this.hasCachedData(this.cachedRequestsData.articleCards)) {
            this.__articleCardsPromise.__completeWithValue(
                this.cachedRequestsData.articleCards
            );
        } else {
            this.getBlogData().then((value) => {
    ...
    private getSearchItems() {
        if (this.hasCachedData(this.cachedRequestsData.searchItems)) {
            this.__searchItemsPromise.__completeWithValue(
                this.cachedRequestsData.searchItems
            );
        } else {
            this.getBlogData().then((value) => {
    ...
    private getBannerBlogCards() {
        if (this.hasCachedData(this.cachedRequestsData.bannerBlogCards)) {
            this.__bannerBlogCardsPromise.__completeWithValue(
                this.cachedRequestsData.bannerBlogCards
            );
        } else {
            this.getBlogData().then((value) => {
