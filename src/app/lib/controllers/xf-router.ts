...
export class XFRouter implements IXFRouter {
    ...
    public onDidNavigate: IAsyncGlobalEventEmitter<RouterLocation> =
        CommonEvents.createGlobalAsyncEventEmitter();
    ...
    public getInitialRouterParamsAndOptions(): Promise<RouterLocation> {
        return new Promise((c) => {
            if (this.vaadinRouterInstance) {
                ...
                formatErrorMessage('Accessing Router Param and Options With Undefined Router Instance','XF-ROUTER-MODEL')
            }
        });
    }
